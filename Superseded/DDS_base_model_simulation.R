...
Battery$Max_Wl_Battery[Battery$Max_Wl_Battery==0] <- NA
Battery$Max_Wl_Battery[is.na(Battery$Max_Wl_Battery)] <- mean(Battery$Max_Wl_Battery, na.rm = T) #mean value is assigned for missing value for now, should be scaled according to nearby stations later*
...
for (j in 305:305)#1:length(fips$V1))
{
  #Read in index
  ...
  for (i in 1:numIter)
  {
    ...
    #Print to console
    print_str = paste("Sim:", sim, "Tract:", fips$V1[j],"RMSE BEST",i,":",RMSE_best)
    print(print_str)
  ...
  x_bestt=t(x_best)
  colnames(x_bestt) <- c("Surge_threshold","Rain_threshold","alpha_d","alpha_a","alpha_p","alpha_r","mew_a","mew_p","U_rate","POT_S_max","POT_R_max")
  
  Results_SH = SocHydModel_SurgePrecip(W = Battery$Max_Wl_Battery[10:52], Precip = precip$precip_max[10:52], x_best[1], x_best[2], x_best[3], x_best[4], x_best[5], x_best[6], 1, x_best[7], x_best[8], 
                                        1, 1, 1, 1, x_best[9], x_best[10], x_best[11])
 ...
 # write.table(Coeff_SH_mat,paste("C:/SANDEEP/SocioHydro_Model/correlation_check/Coeff_SH_matrix.csv"),sep=",")
 # write.table(Results_SH_mat,paste("C:/SANDEEP/SocioHydro_Model/correlation_check/Results_SH_matrix.csv"),sep=",")

}
#correlation matrix between SH coeffiecients
...
#preparedness
plot(Battery$Year[10:52] ,Results$P, type = "o" )
