...
SocHydModel_SurgePrecipHousing <- function(
        ...
        #housing
        phi = 0.8, #factor by which housing price drops after loss, phi ranges from 0 to 1?
        HPmax = 1
){
    #Initialize vectors
    ...
    #housing
    HP = rep(HPmax,length(W)) #what should be our initial HP values ?
    tflood = rep(0,length(W))
    Sloss = rep(0,length(W))
    ...
    for (t in 2:length(W))
    {
        ...
        else
        {
            ...
            HP[t] = HP[t-1]*phi #Reduces S by a factor phi (However in this case phi is independent to loss so the drop in housing price becomes independent of the magnitude of loss ?)
    ...
    SocHydResults<-data.frame(W,L,D,R,U,A,P,HP)
    return(SocHydResults)
}

#Socio-hydrological model test    
# Results = SocHydModel_SurgePrecipHousing(W = Battery$Max_Wl_Battery, Precip = precip$precip_max,
#                                  2, 0.12, 6, 13, 9, 0.03,	0.012, 0.001,0.01,
#                                  1, 1, 1, 1, 0.5 ,0.14, 0.1, 0.8,1)
